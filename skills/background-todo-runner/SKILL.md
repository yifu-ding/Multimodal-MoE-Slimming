...
After the user authorizes an independent background Codex executor, use a separate detached supervisor with a single-instance lock and persistent state. Preserve the agreed inspection cadence (MAES uses 5-minute lightweight fault checks and 30-minute Markdown progress reports); healthy checks must be shell-only, without invoking Codex. The bundled shell supervisor defaults to monitoring only (`AUTO_RESTART=0`); do not enable blind restarts alongside a reasoning executor.

Invoke Codex for unfinished work when the worker stops, or when current-stage logs and heartbeats indicate sustained stagnation. GPU occupancy need not block diagnosis, but verify ownership and resource availability before resuming any GPU work. Treat failed status checks as unknown, not as proof of a stopped worker. For live-but-stale workers, diagnose ownership and the blocking cause; never kill a process merely because a timestamp is old.
...
When the user authorizes skipping failures to keep work moving (as in MAES), apply that three-round limit per model/ratio, not globally. Isolate the exhausted configuration with its evidence and continue other unfinished independent configurations. Within a configuration, a failed benchmark must not block independent benchmarks or Judge inputs that are available. Missing/invalid Scores or plans still block their dependents; never bypass validation. Track skipped items as missing/failed, not complete. Only stop the remaining campaign when every remaining configuration is exhausted or a genuinely global permission/credential/resource boundary prevents progress.
