...
for task in "${tasks[@]}"; do
    for ratio in p30 p50; do
        ...
        env -u LIMIT -u FORCE \
            "${video_reader_env[@]}" \
            MASK_PLAN="${ROOT}/runtime/mask_plans/kimi-${ratio}-direct.pt" \
            RUN_DIR="${ROOT}/results/vllm_ours/kimi/direct-mask/${ratio}-random-half-seed42" \
            RESPONSE_CACHE_ROOT="${ROOT}/results/vllm_ours/kimi/direct-mask/${ratio}-full/response_cache" \
            RANDOM_SUBSET_FRACTION=0.5 RANDOM_SUBSET_MIN_SAMPLES=500 RANDOM_SUBSET_SEED=42 \
            BASELINE_LABEL="kimi_vl_direct_mask_${ratio}" \
...
for ratio in p30 p50; do
    if /home/dyf/miniconda/envs/vllm-maes/bin/python -c \
        'import sys; sys.path.insert(0,".automation/kimi_direct"); from campaign import valid_judge; sys.exit(0 if valid_judge(sys.argv[1]) else 1)' "${ratio}"; then
        continue
    fi
    printf '%s\n' "judge ${ratio}" > "${STATE}/current.txt"
    echo "[judge] $(date --iso-8601=seconds) ${ratio}"
    PREDICTIONS_DIR="${ROOT}/results/vllm_ours/kimi/direct-mask/${ratio}-full" \
        JUDGE_RANDOM_SUBSET_FRACTION=0.5 JUDGE_RANDOM_SUBSET_MIN_SAMPLES=500 JUDGE_RANDOM_SUBSET_SEED=42 \
