...
def _tmp_media_root() -> str:
    root = os.path.join(".cache", "modes_media_cache")
...
def _resolve_longvideobench_video_path(video_path: str) -> str:
    configured_root = os.environ.get("LONGVIDEOBENCH_ROOT", "").strip()
    candidates = [
    ...
    if configured_root:
        candidates.insert(0, os.path.join(configured_root, "videos", video_path))
    extracted = _find_existing_media_path(candidates)
    if extracted:
        return extracted
    raise FileNotFoundError(
        f"LongVideoBench video not found for {video_path}. "
        "Set LONGVIDEOBENCH_ROOT or place extracted videos under "
        "$HF_HOME/datasets/longvideobench."
    )
...
def load_eval_task(task_name: str) -> tuple:
    ...
    return load_fn(), build_helpers_fn()
