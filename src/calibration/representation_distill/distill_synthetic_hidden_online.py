...
def _collect_stream_examples(
    *,
    ...
    while collected < target_count:
        ...
        hidden, labels, position_ids = _extract_teacher_chunk(
            bundle=bundle,
            raw_batch=raw_batch,
...
def _compute_pooled_next_block_rel_l2(
    *,
    teacher_next_block: torch.Tensor,
    teacher_attention_mask: torch.Tensor,
    synthetic_next_block: torch.Tensor,
    synthetic_attention_mask: torch.Tensor,
) -> torch.Tensor:
    teacher_flat = teacher_next_block.float()[teacher_attention_mask.bool()]
    synthetic_flat = synthetic_next_block.float()[synthetic_attention_mask.bool()]
    if teacher_flat.numel() == 0 or synthetic_flat.numel() == 0:
        return torch.tensor(0.0, device=teacher_next_block.device)

    teacher_mean = teacher_flat.mean(dim=0)
    synthetic_mean = synthetic_flat.mean(dim=0)
    teacher_var = teacher_flat.var(dim=0, unbiased=False)
    synthetic_var = synthetic_flat.var(dim=0, unbiased=False)

    mean_rel = (synthetic_mean - teacher_mean).norm() / teacher_mean.norm().clamp_min(1e-6)
    var_rel = (synthetic_var - teacher_var).norm() / teacher_var.norm().clamp_min(1e-6)
    return 0.5 * (mean_rel + var_rel)
...
def main() -> None:
    ...
    from src.calibration.common import load_model_bundle, resolve_model_name_or_path
    ...
    for step in progress:
        ...
        teacher_batch_cpu, teacher_batch_labels_cpu, teacher_position_ids_cpu = _collect_stream_examples(
            stream=stream,
        ...
        teacher_position_ids = teacher_position_ids_cpu.to(device=device, dtype=torch.long)
        teacher_attention_mask = torch.ones(teacher_batch.shape[:2], dtype=torch.long, device=device)
        ...
        # block 监督: 训练期对 teacher / synthetic 两边现算 next-block 输出, 再做 pooled relative L2
        if args.lambda_block > 0:
            with torch.inference_mode():
                teacher_next_block = _forward_next_block(
                    bundle=bundle,
                    hidden_states=teacher_batch,
                    attention_mask=teacher_attention_mask,
                    position_ids=teacher_position_ids,
                    teacher_layer=args.teacher_layer,
                    target_dtype=next_block_dtype,
                ).to(dtype=train_dtype)
            synthetic_next_block = _forward_next_block(
                bundle=bundle,
                hidden_states=synthetic_hidden,
                attention_mask=synth_batch_attention_mask,
                position_ids=synth_batch_position_ids,
                teacher_layer=args.teacher_layer,
                target_dtype=next_block_dtype,
            )
            losses["block_rel_l2"] = _compute_pooled_next_block_rel_l2(
                teacher_next_block=teacher_next_block,
                teacher_attention_mask=teacher_attention_mask,
                synthetic_next_block=synthetic_next_block,
        ...
        del teacher_batch, teacher_batch_labels, teacher_position_ids, teacher_attention_mask
        if args.lambda_block > 0:
            del teacher_next_block, synthetic_next_block
