    ...
    if args.selection_mode == "joint":
        ...
        _save_plot_data(
            plot_data_path,
            {
                ...
                "ranking": {
                    "core_desc": torch.from_numpy(np.argsort(metrics["core_score"])[::-1].copy()).to(torch.int64),
                    "outlier_desc": torch.from_numpy(np.argsort(metrics["outlier_score"])[::-1].copy()).to(torch.int64),
                },
                "defaults": {
        ...
        _save_plot_data(
            plot_data_path,
            {
                ...
                "ranking": {
                    "text_core_desc": torch.from_numpy(np.argsort(text_core_score)[::-1].copy()).to(torch.int64),
                    "text_outlier_desc": torch.from_numpy(np.argsort(text_outlier_score)[::-1].copy()).to(torch.int64),
                    "visual_core_desc": torch.from_numpy(np.argsort(visual_core_score)[::-1].copy()).to(torch.int64),
                    "visual_outlier_desc": torch.from_numpy(np.argsort(visual_outlier_score)[::-1].copy()).to(torch.int64),
                },
                "defaults": {
                    "core_count": int(core_count),
                    "outlier_count": int(outlier_count),
