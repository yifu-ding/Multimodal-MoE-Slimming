...
## E0b：不可达 floor 重试与 EP 扩展（2026-09-22 16:30 CST）

在修改论文前，又补充了 31 个 case，仓库结果见 `artifacts/placement-e0-followup/summary.md`：

| 组别 | 数量 | 结果 |
| --- | ---: | --- |
| L=4 floor retry | 24 | 24/24 触发 retry，逐 quantum 搜索的第一个可行 target 全部精确复现旧 MILP 最优值 |
| m=4, floor=0 | 5 | 5/5 在 0.108--0.344 s 找到 spread=0 并证明最优 |
| p=0.5, L=48, m=32 | 1 | 1.184 s 找到 spread=0 并证明最优 |
| p=0.5, L=48, m=64 | 1 | 110.746 s 找到 spread=0 并证明最优 |
